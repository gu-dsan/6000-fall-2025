...
1. In your student email inbox, you will have an email from _Microsoft Azure <azure-noreply@microsoft.com>_ with the subject _You’ve been asked to accept Azure subscription ownership_.
![](img/azure-step1.png)
1. Open the email.
...
The best strategy for the next step if you are in the classroom with others is to stagger the clicking of _Accept_ because if too many people do it at once there is a high possibility of errors. Coordinate amongst yourselves to do **no more than 10 at a time.**
:::
...
1. **Log out of Azure and log back in to ensure that everything us updated.**
...
1. [Sign in](#sign-in-to-the-azure-portal) to the Azure portal if you are not already signed in
...
3. Click on the `Subscriptions` oval and select _Show all subscriptions_
4. Uncheck the checkbox that says _Show only subscriptions selected in the global subscriptions filter_
4. You will see a subscription called `DSAN6000 Fall 2024 Individual_XXXXX_YYYYY` where `XXXX` is your first name, and `YYYY` is your last name.
...
2. Type `ssh keys` in the search bar, and click on _SSH Keys_ service
![](img/azure-ssh-01.png)
3. Click _Create_
...
![](img/azure-ssh-02.png)

1. In the **Upload key** box, paste the contents of your **public key**:
	- `~/.ssh/id_rsa.pub` (Mac/Linux)
	- `C:\Users\<your_name>\.ssh\id_rsa.pub` (Windows)
2. Click _Review + create_
2. After validation passes, click _Create_
1. Once the SSH Key is uploaded, you'll see it in the SSH Key list (if your right subscriptions are selected in the filter). You may have to hit **"Refresh"** on your browser for the page to update with your ssh key.

![](img/azure-ssh-03.png).
...
:::callout-note
The IP address _may_ change every time you start the same virtual machine from a stopped (paused) state.
:::
...
2. To start again in the future, click on _Restart_, though the IP address _may_ change
...
1. Go to the VM Overview page for the VM you just created. You can get there multiple ways.
2. Click on _Delete_
1. Select _all_ associated resources to the VM (OS Disk, Network interfaces, and Public addresses) to be deleted
...
6. Select the [resource group](#create-an-azure-resource-group) you crated from the dropdown
